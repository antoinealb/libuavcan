...
const uavcan::uint32_t USecPerOverflow = (1000000 / TIMX_INT_FREQ);
...
void init()
{
    ...
    putreg32(TIMX_INPUT_CLOCK / TIMX_INT_FREQ - 1, TMR_REG(KINETIS_PIT_LDVAL_OFFSET));
...
static uavcan::uint64_t sampleUtcFromCriticalSection()
{
    ...
    volatile uavcan::uint32_t cnt = USecPerOverflow - getreg32(TMR_REG(KINETIS_PIT_CVAL_OFFSET));

    if (getreg32(TMR_REG(KINETIS_PIT_TFLG_OFFSET)) & PIT_TFLG_TIF)
    {
        cnt = USecPerOverflow - getreg32(TMR_REG(KINETIS_PIT_CVAL_OFFSET));
