...
int CanIface::init(const uavcan::uint32_t bitrate, const OperatingMode mode)
{
    ...
    if (self_index_ == 0)
    {
        volatile uint32_t *idFilterRegion = (volatile uint32_t *)(&can_->MB[flexcan::FirstFilter].mb);
        volatile uint32_t *idFilterEnd = (volatile uint32_t *)(&can_->MB[flexcan::NumMBinFiFoAndFilters - 1].mb);
        while(idFilterRegion <= idFilterEnd)
        {
            *idFilterRegion++ = 0;
        }
        can_->RXFGMASK = 0;
